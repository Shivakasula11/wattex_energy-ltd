...
const Home = () => {
  return (
    <>
        ...
        schema={[
          {
            ...
            description:
              "Leading solar energy solutions provider in Hyderabad offering residential, commercial, and industrial solar installations.",
      ...
      <section
        id="home"
        className="relative min-h-[100svh] h-[100vh] flex items-start sm:items-center pt-48 xs:pt-52 sm:pt-0 overflow-hidden"
      >
        <div className="absolute inset-0 z-0">
          <img
            ...
            alt="Solar panels installation on rooftop in Hyderabad by Wattex Energy"
          />
        ...
        <div className="container mx-auto px-3 xs:px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-4xl space-y-4 xs:space-y-5 sm:space-y-7 md:space-y-8 lg:space-y-10 animate-slide-left">
            <h1 className="text-5xl xs:text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-black text-solar-primary font-heading leading-[1.1] tracking-tighter break-words">
              Energy For a<br />
              <span className="text-solar-primary uppercase">
                Sustainable <br /> Future
              </span>
            </h1>
            <p className="text-white/70 text-base xs:text-lg sm:text-base md:text-lg lg:text-xl xl:text-2xl max-w-2xl font-medium leading-relaxed">
              Industrial-grade renewable energy solutions engineered for
              zero-carbon impact and maximum economic yield.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 xs:py-16 sm:py-20 md:py-24 lg:py-28 xl:py-32 bg-slate-50">
        <div className="container mx-auto px-3 xs:px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8 xs:mb-10 sm:mb-12 md:mb-14 lg:mb-16 space-y-2 xs:space-y-3 sm:space-y-4">
            <span className="text-solar-primary font-black uppercase tracking-widest text-[9px] xs:text-[10px] sm:text-xs">
              Why Solar?
            </span>
            <h2 className="text-xl xs:text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-solar-deep tracking-tight">
              The Smart Switch
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 xs:gap-5 sm:gap-6 md:gap-7 lg:gap-8 max-w-4xl mx-auto">
            ...
            <div className="rounded-2xl xs:rounded-3xl sm:rounded-[2rem] md:rounded-[2.5rem] overflow-hidden border border-slate-200 bg-white">
              <div className="bg-slate-100 px-3 xs:px-4 sm:px-6 md:px-8 py-3 xs:py-4 sm:py-5 border-b border-slate-200">
                <span className="text-[9px] xs:text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-widest text-slate-500">
                  Without solar
                </span>
              </div>
                ...
                {[
                  ...
                  <div key={i}>
                    <div className="flex justify-between items-center gap-2 py-2 xs:py-2.5 sm:py-3">
                      <span className="text-slate-500 font-medium text-[10px] xs:text-xs sm:text-sm">
                        {row.label}
                      </span>
                      <span
                        className={`font-bold text-[10px] xs:text-xs sm:text-sm text-right ${row.neutral ? "text-slate-400" : "text-red-600"}`}
                      >
                        {row.value}
                      </span>
                    </div>
            ...
            <div className="rounded-2xl xs:rounded-3xl sm:rounded-[2rem] md:rounded-[2.5rem] overflow-hidden border-2 border-emerald-500 bg-white shadow-xl shadow-emerald-100">
              <div className="bg-emerald-50 px-3 xs:px-4 sm:px-6 md:px-8 py-3 xs:py-4 sm:py-5 border-b border-emerald-100 flex flex-col xs:flex-row justify-between items-start xs:items-center gap-2 xs:gap-3">
                <span className="text-[9px] xs:text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-widest text-emerald-800">
                  With Wattex solar
                </span>
                ...
                {[
                  ...
                  <div key={i}>
                    <div className="flex justify-between items-center gap-2 py-2 xs:py-2.5 sm:py-3">
                      ...
                      <span className="font-bold text-[10px] xs:text-xs sm:text-sm text-emerald-600 text-right">
                        {row.value}
                      </span>
                    </div>
                    {i < 5 && <div className="h-px bg-slate-100" />}
